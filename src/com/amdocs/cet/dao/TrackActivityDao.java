...
public class TrackActivityDao {
	
	public static List<UserActivity> getUserActivities(int userId) {
	    ...
	    String query = "SELECT ua.user_activity_id, ua.user_id, ua.activity_category_id, ua.input, ua.activity_date, ua.total_emission, " +
	                   ...
	                   "JOIN ActivityCategories ac ON ua.activity_category_id = ac.activity_category_id " + // Correct join with activity_category_id
	                   "JOIN Activities a ON ac.activity_id = a.activity_id " + // Join to get the main activity data
	                   "WHERE ua.user_id = ?";
